...
class GitlabProject:
    """ 
    Klasa generująca pliki terraform dla projektu na podstawie zebranych danych z gitlab API.
    """
    ...
    tf_module_gitlab_project_source: str

    def __init__(self, template_path: Path, project: dict, repository_path: Path, logger: Logger = None) -> None:
        ...
        self.__sanitized_group_name = self.__project['name'].replace(".", "_")
        self.__logger.trace(f"  Sanitized group name: {self.__sanitized_group_name}")
        
        self.__project['avatar'] = str(os.path.basename(urlparse(self.__project['avatarUrl']).path)).replace(".png", "")
        self.__project['avatar'] = None if self.__project['avatar'] == "b''" else self.__project['avatar']
        self.__logger.trace(f"  Change avatar name: {self.__project['avatar']}")

        self.tf_module_gitlab_project_source = Config().tf_module_gitlab_project_source

    def __save_module(self, body: Union[str,dict], is_json: bool = False):
        dest_tf = self.__repository_path / f"{self.__project['fullPath']}.tf"
        dest_tf_json = self.__repository_path / f"{self.__project['fullPath']}.tf.json"
    ...
    def generate_hcl(self, force: bool=False) -> None:
        ...
        for j2_path in list(self.__template_path.glob("*.tf.j2")):
            ...
            rendered = template.render({
                ...
                'source': self.tf_module_gitlab_project_source
            })

            if template_name == 'module.tf.j2':
                self.__save_module(body=rendered, is_json=False)
            else:
                dest = self.__repository_path / f"{self.__project['fullPath']}" / template_name.replace('.j2','')
